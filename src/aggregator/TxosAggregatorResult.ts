export class TxosAggregatorResult {
  private readonly nbCmbn: number;
  private readonly matLnkCombinations: number[][] | null;
  ...
  getMatLnkCombinations(): number[][] | null {
  ...
  computeMatLnkProbabilities(): number[][] | null {
    ...
    const matLnkProbabilities = Array.from<number[]>({ length: this.matLnkCombinations.length });
    if (this.nbCmbn > 0) {
